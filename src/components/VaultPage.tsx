...
const VaultPage = () => {
  ...
  // Menu - centered folder icon
  return (
    <section className={`min-h-screen flex flex-col items-center justify-center relative overflow-hidden page-transition ${isTransitioning ? 'animate-[pageFadeOut_0.3s_ease-in_forwards]' : ''}`}>
      <div className="absolute inset-0 bg-background" />
      <img
        src={vaultLamp}
        alt=""
      ...
      <div className="relative z-10 flex flex-col items-center gap-3">
        <button
          ...
          className="group flex flex-col items-center gap-2 cursor-pointer transition-transform duration-300 hover:scale-110 active:scale-95 fade-in-up"
        >
          <Folder
            size={64}
            className="text-[#5AC8FA] drop-shadow-[0_4px_20px_rgba(90,200,250,0.4)] group-hover:drop-shadow-[0_4px_30px_rgba(90,200,250,0.6)] transition-all duration-300"
            fill="currentColor"
          />
          <span className="font-mono text-xs text-muted-foreground tracking-wider">projects</span>
        </button>
      </div>
    </section>
  );
